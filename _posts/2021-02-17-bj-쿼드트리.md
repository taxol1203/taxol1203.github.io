...
## 접근 방법
분할 정복 문제이다. 3가지 
1. 먼저 2<sub>n</sub>x2<sub>n</sub>배열이 0 또는 1로만 이루어져 있는지 확인한다. 같은 정보로 이루어져 있으면, 해당하는 정보를 출력하고 종료한다.  
2. 만약 정보가 섞여있다면, 2<sub>n</sub>x2<sub>n</sub> 크기의 배열을 4개의 2<sub>n-1</sub>x2<sub>n-1</sub>배열로 나누어 다시 탐색한다.  
